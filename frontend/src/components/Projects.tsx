      ...
      <div className="flex flex-col gap-4">
        {projects.map((project, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="rounded-2xl p-[1px] bg-gradient-to-r from-transparent via-white/10 to-transparent hover:via-blue-500/50 transition-all duration-500"
          >
            <div 
              className="bg-navy rounded-2xl p-6 cursor-pointer"
                  ...
                  >
                    ...
                    <div className="mt-4 flex gap-2">
                      {project.tech.split(', ').map(t => (
                        <span key={t} className="text-xs bg-slate-800 px-2 py-1 rounded">{t}</span>
                      ))}
                    </div>
                  </motion.div>
                )}
